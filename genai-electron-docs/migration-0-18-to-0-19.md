...
the default strategy. Callers who supplied their own `combos` keep the singular `profile` and need
no change.
...
- `status` is a terminal outcome: `complete`, `budget-exhausted`, `no-viable-candidate`, `aborted`,
  or `failed`.
...
- `probe.resourceRegime` records the settled resource level a launch was measured under. A confirmed
...
2. Leave `profile` + `combos` callers unchanged.
3. Discard persisted v0.18 calibration reports and recalibrate.
4. Narrow report and progress handling on `strategy`, and read `status` before `selected`.
5. Treat adaptive recommendations as conditional on the pinned MoE placement.
6. Update the declared dependency range to admit `0.19.0`.
