...
### Fixes:
1. **[HIGH] Version 0.3.0 → 0.5.0** — Update header version + phase description in `index.md`. Fix `@version` in `src/index.ts` line 9. Also fix in `README.md` and `docs/SETUP.md`.
...
1. Run `npm run build` to ensure code changes compile
2. Run `npm test` to ensure no regressions
3. Grep for remaining "0.3.0" references
4. Grep for remaining "zero runtime dependencies" claims
5. Grep for remaining `(error: Error)` in crashed event handlers across all docs
6. Review all changed docs for internal cross-doc consistency
