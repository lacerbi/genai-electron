# Token Truncation Issue: Responses Capped Below max_tokens
...
**The Problem:**
- genai-electron set `parallelRequests: 8` based on CPU core count
- llama.cpp's KV cache is **shared across all parallel slots**
- With parallelRequests > 1, the available context is split across slots
- Result: Responses capped at much lower than the full contextSize

**Observed Behavior:**
- contextSize 1024 → responses capped at ~256 tokens
- contextSize 2048 → responses capped at ~512 tokens
- The exact mechanism is unclear, but KV cache splitting was the cause

**Why this was wrong for genai-electron:**
- The CPU-based parallelRequests logic was designed for multi-user servers
- genai-electron targets single-user Electron apps (one request at a time)
- Setting parallelRequests > 1 splits the cache unnecessarily

**The Fix:**
- Changed `SystemInfo.recommendParallelRequests()` to return **1** (File: `src/system/SystemInfo.ts:330-335`)
- Result: Full contextSize available for each request
- Example: contextSize 1024 → 1024 tokens available (not ~256)

---

## Issue Summary

Responses from llama-server were being truncated well below the requested `max_tokens` parameter.

### Observed Pattern

| Model | contextSize | Expected max_tokens | Actual cap |
|-------|-------------|---------------------|------------|
| Qwen 3 4B Instruct | 1024 | 800-4096 | ~243-256 |
| Qwen (smaller) | 2048 | 800-4096 | ~512 |

**Key observation:** Responses were capped well below the requested max_tokens, regardless of the value provided.
