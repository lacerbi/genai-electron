...
description: Determine and execute a patch or minor genai-electron release through its release-preparation metadata and migration guide, pull request, CI, annotated tag, and GitHub release workflow. INVOKE ONLY when the user explicitly runs /release-genai-electron, names this skill, or directly asks to execute the repository release process — including phrasing like "let's release", "proceed with the release", or "ship it". Do not invoke for general questions about versions, tags, pull requests, npm, or release planning.
